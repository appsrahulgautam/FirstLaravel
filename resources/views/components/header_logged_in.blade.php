...
<html lang="en">
  ...
  <body>
    <header class="header-bar mb-3">
      <div class="container d-flex flex-column flex-md-row align-items-center p-3">
        ...
        <div class="flex-row my-3 my-md-0">
          ...
          <a class="btn btn-sm btn-success mr-2" href="/createpost">Create Post</a>
          <form action="/logoutman" method="GET" class="d-inline">
    ...
    <p class="alert {{ Session::get('alert-class', 'alert-info') }}">{{ Session::get('successcustom') }}</p>
    @endif

    @if(Session::has('failedcustom'))
    <p class="alert {{ Session::get('alert-class', 'alert-info') }}">{{ Session::get('failedcustom') }}</p>
    @endif
